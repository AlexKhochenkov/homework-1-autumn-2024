...
3. Боковое меню
4. Меню редактирования папок
5. Редактирование наличия письма в папке
6. Профиль
7. Страница списка писем
8. Страница письма
9. Блок Профиль
10. Страница написания и редактирования письма

# [Страница авторизации](https://mailhub.su/login)

## [Меню входа](https://mailhub.su/login)

![Меню входа](images/auth_image_1.png)
* При нажатии на ссылку "Создание ящика" происходит переход на [страницу регистрации](https://mailhub.su/signup)
* При вводе отправке пустого поля "Имя ящика", отображается надпись "Введите имя ящика"
* Если при вводе отправке поля "Имя ящика" превышает 64 символа, отображается надпись "Слишком длинное имя ящика"
* При вводе отправке неверного поля "Имя ящика", содержащего символы, не относящиеся к латинскому алфавиту (от a до z и от A до Z) и цифрам (от 0 до 9), отображается надпись "Некорректное имя ящика"
* При вводе отправке пустого поля "Пароль", отображается надпись "Введите пароль"
* Если при вводе отправке поля "Пароль" превышает 256 символа, отображается надпись "Слишком длинный пароль"
* При вводе верного "Имя ящика" и неверного пароля и последующем нажатии на кнопку "Войти", отображается надпись "Неверное имя ящика и/или пароль"
* При вводе неверного "Имя ящика" и верного пароля и последующем нажатии на кнопку "Войти", отображается надпись "Неверное имя ящика и/или пароль"
* При вводе "Имя ящика" и пароля, принадлежащим ранее зарегистрированному аккаунту, и последующем нажатии на кнопку "Войти", пользователя перенаправляет на [страницу писем](https://mailhub.su/main)


# [Страница регистрации](https://mailhub.su/signup)

![Меню регистрации](images/registration.jpg)
* При нажатии на ссылку "Вход в аккаунт" происходит переход на [страницу авторизации](https://mailhub.su/login)
* При вводе и отправке пустых полей "Имя", "Фамилия", "Имя ящика", "Пароль", "Пароль ещё раз", соответственно отображается надпись "Введите имя", "Введите фамилию", "Введите имя ящика", "Введите пароль", "Введите пароль ещё раз"
* Если при вводе и отправке поля "Имя" превышает 64 символа, отображается надпись "Слишком длинное имя"
* Если при вводе и отправке поля "Фамилия" превышает 64 символа, отображается надпись "Слишком длинная фамилия"
* Если при вводе отправке поля "Имя ящика" превышает 64 символа, отображается надпись "Слишком длинное имя ящика"
* При вводе и отправке неверного поля "Имя ящика", содержащего символы, не относящиеся к латинскому алфавиту (от a до z и от A до Z) и цифрам (от 0 до 9), отображается надпись "Некорректное имя ящика"
* Если при вводе и отправке поля "Пароль" превышает 256 символа, отображается надпись "Слишком длинный пароль"
* Если при вводе и отправке поля "Пароль" меньше 8 символов, отображается надпись "Минимальная длина 8 символов"
* При вводе и отправке поля "Пароль ещё раз", значение которого не совпадает с полем "Пароль", отображается надпись "Пароли не совпадают"
* При нажатии на кнопку "Пол" выбранный пол (Мужчина/Женщина) меняется на противоположный
* При вводе и отправке полей "Дата рождения", значение которых превосходит сегодняшнее число, отображается надпись "Дата рождения превосходит сегодняшнее число"
* При вводе и отправке полей "Дата рождения", где число превосходит верхнюю границу чисел в месяце, отображается надпись "В этом месяце нет такого числа"
* При вводе и отправке полей "Дата рождения", где дата имеет значение 29 февраля в невисокосном году, отображается надпись "В этом месяце в невисокосном году нет такого числа"
* При вводе корректных данных, и последующем нажатии на кнопку "Создать", ящик пользователя регистрируется, и пользователя перенаправляет на [страницу писем](https://mailhub.su/main)
...
* Если в папке есть непрочитанные письма, то напротив названия папки появляется количество этих сообщений
...
* При вводе небольшого количества символов в названии папки надпись корректно отображается. Например название "www" будет отображаться как "www"
* При достаточно длинном названии папки часть названия скрывается. Например название "fffffffffffffffff" будет отображаться как "fffffffffffff..."

<img width="200" alt="image" src="https://github.com/user-attachments/assets/f5deedda-cdcc-4196-b9b4-38d7e8ef01db">
...
* Если пользователь находится в каком угодно месте, только не на странице писем данной папки, то происходит удаление папки, при этом пользователь остаётся на той же странице, на которой был
* Если пользователь находится на странице писем данной папки, то при удалении пользователь остается на той же странице
...
* Если папок нет, то выпадает надпись "Нет папок!"
* Если письмо уже находится в единственно созданной папке, то выпадает надпись "Нет папок!"
...
* Если длина поля "Имя" больше 64 символов, то появляется сообщение "Слишком длинное имя".

* Если длина поля "Фамилия" больше 64 символов, то появляется сообщение "Слишком длинная фамилия".

* Если длина поля "Отчество" больше 64 символов, то появляется сообщение "Слишком длинное отчество".

* В случае если поле "Номер телефона" не пустое, но содержат что-либо кроме цифр выдается ошибка "Некорректный номер".

* Если длина поля "Номер телефона" больше 32 символов, то появляется сообщение "Слишком длинный номер".

* В случае если поле "О себе" не пустое, оно может содержать любые символы.

* Если длина поля "О себе" больше 64 символов, то появляется сообщение "Слишком много текста".
...
* При нажатии на кнопку "Сохранить" происходит замена текста в кнопке на "Успешно сохранено", данные сохраняются.

* Если в каком-либо из полей "Имя" или "Фамилия" или "Отчество" или "О себе" или "Номер телефона" присутствует ошибка, то при нажатии на кнопку "Сохранить" указываются ошибки под полями.
...
* При нажатии на кнопку "Удалить ящик" всплывает плашка "Вы уверены, что хотите удалить ящик?", чтобы пользователь подтвердил действие. Если пользователь отказывается, то плашка "Вы уверены, что хотите удалить ящик?" убирается.
![image_4.png](images/profile_image_4.png)

## Изменение аватарки пользователя (нажатие по аватарке/кнопке)

* При нажатии на аватарку отображается окно изменения аватара

* При нажатии на кнопку отображается окно изменения аватара
...
* При выборе изображения с поддерживаемыми типами (`.jpg`, `.jpeg`, `.jfif`, `.pjpeg`, `.pjp`, `.png`) и весом менее 5МБ аватар обновляется, появляется новая фотография.

* При выборе изображения с поддерживаемыми типами (`.jpg`, `.jpeg`, `.jfif`, `.pjpeg`, `.pjp`, `.png`) и весом более 5МБ аватар не обновляется, под аватаром отображается предупреждение "Файл превышает максимальный размер 5 МБ".

* При выборе изображения типами, которые не соответствуют следующим (`.jpg`, `.jpeg`, `.jfif`, `.pjpeg`, `.pjp`, `.png`) и весом менее 5МБ аватар не обновляется, под аватаром отображается предупреждение "Некорректный формат файла".
...
* Отправитель, тема, текст у писем из списка писем могут отображаться не полностью, если ширина окна браузера недостаточна для полного отображения
...
* Список писем способен загрузиться и отрендериться полностью, если содержит менее 200 писем
...
# Страница списка отправленных писем

![image](https://github.com/user-attachments/assets/8631b965-3384-4a55-8b2f-b10cccde97ac)
![image](https://github.com/user-attachments/assets/9ba8cd94-5370-4b38-8e07-92a47bda57d2)

## Список писем

![image](https://github.com/user-attachments/assets/0e3ea4cc-48d1-4b1b-b265-e95d629e593f)

### Общее

* Содержит отправленные письма
* Информация, которую можно узнать о письмах в списке писем: статус прочтения, получатель, аватар получателя, тема, текст, дата отправления.
* Получатель, тема, текст у писем из списка писем могут отображаться не полностью, если ширина окна браузера недостаточна для полного отображения
* Через список писем можно выбрать нужное письмо и выполнить с ним следующие действия: открыть письмо, отметить прочитанных/непрочитанным, выбрать
* Одинаковый функционал веб и тач версии списка писем
* Список писем способен загрузиться и отрендериться полностью, если содержит менее 200 писем
...
![image](https://github.com/user-attachments/assets/e899e1ca-0857-4fda-b36a-ef9a31ba0e8f)
![image](https://github.com/user-attachments/assets/1e7128ff-fe2c-4c5c-8f88-f34821769cca)
...
* В контейнере письма отображается вся доступна пользователю информация о письме: статус прочтения, отправитель, аватар отправителя, тема, текст, дата отправления, вложения. В случае отображения списка отправленных, отправитель и аватар отправителя меняются на получателя и аватар получателя соответственно
...
* Нажатие выполняет редирект предыдущую страницу в истории
* Ничего не происходит, если предыдущей страницы нет
